...
app.get("/api/titles", requireAuth, async (req, res) => {
  ...
  if (genre) {
    filter.genres = { $regex: `^${escapeRegex(String(genre))}$`, $options: "i" };
  }
...
app.get("/api/titles/:id", requireAuth, async (req, res) => {
  try {
    ...
    const title = {
      ...
      posterPath:
        t.posterPath ||
        (t.posterFileId ? `/img/${t.posterFileId}` : "/img/placeholder.jpg"),
...
app.get("/api/recommendations", requireAuth, async (req, res) => {
  ...
  const titles = await Titles.find({
    ...(topGenres.length
      ? { genres: { $in: topGenres.map((g) => new RegExp(`^${escapeRegex(g)}$`, "i")) } }
      : {}),
    _id: { $nin: likedObjIds },
  })
    .sort({ createdAt: -1 })
...
app.get("/api/genres/shelves", requireAuth, async (req, res) => {
  const minHours = Math.max(1, parseInt(req.query.minHours || "1", 10)); // מחריג שעה אחרונה
  ...
  const rows = await Titles.aggregate([
    ...
    { $project: { _id: 0, name: "$_id", titles: { $slice: ["$titles", perGenre] } } },
    { $sort: { name: 1 } }
  ]).toArray();
...
app.get("/api/genres/rows", requireAuth, async (req, res) => {
  ...
  const excludeHours = Math.max(0, parseInt(req.query.exclude_hours || "1", 10));
...
app.get("/media/:id", async (req, res) => {
  try {
    ...
    if (!size) {
      ...
      res.status(200).setHeader("Content-Type", contentType).setHeader("Content-Length", "0").end();
      return;
    }
    ...
    if (range) {
      ...
      const stream = videoBucket.openDownloadStream(_id, { start, end: end + 1 });
